...
- [x] Command palette (`⌘K`): Recent / Go to / Search results / Create — permission-registered
      (`components/shell/commands.ts`). **Not yet:** work-item context actions (change state,
      priority, assign, labels, copy link) — those need a selected-item context the board
      doesn't yet publish to the shell, and `board-view.tsx` renders outside `AppShell`.
...
- [~] **1. Design system / IA gaps** (Prompt A) — palette, recents, density, UX states, My Work
      nav all landed. Outstanding: favourites/pinned views, undo-capable toasts,
      unsaved-change protection, automated a11y (axe) + 200%/320% zoom passes.
- [~] **2. Single-org access control** (Prompt B) — `lib/capabilities.ts` is the canonical
      ...
      27/27). **Module gating completed** — `ai_assistant`/`bookmarks` are now consumed at their
      render sites. Outstanding: membership audit events; `094` still needs applying to prod.
