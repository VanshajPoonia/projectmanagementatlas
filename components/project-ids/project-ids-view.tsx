...
import { Check, Download, Hash, Pencil, Search, X } from 'lucide-react'
...
/** How many upcoming numbers to show above the fold. Preview only — none of these are reserved. */
...
export default function ProjectIdsView({ userId, userName }: { userId: string; userName: string }) {
  ...
  const handleClaim = async () => {
    ...
    setClientName('')
    setCompanyId(null)
    toast.success(`Project ID ${claimed.project_id} is yours`, {
      description: `Recorded for ${claimed.client_name}`,
    })
  }
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              The next numbers in line. Nothing here is reserved until someone grabs it — the number
              you receive is assigned at the moment you click.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {preview.length === 0 ? (
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {preview.map((id, index) => (
                  ...
                  >
                    <span className="font-mono text-base font-semibold tracking-tight">{id}</span>
                    {index === 0 ? (
                  ...
                  {history.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="border-b last:border-0">
                        <td className="py-2 pr-4 font-mono font-semibold">{row.project_id}</td>
