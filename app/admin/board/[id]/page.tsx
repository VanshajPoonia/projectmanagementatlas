...
export default async function BoardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <BoardView board={board} columns={columns || []} users={users || []} isAdmin={profile?.role === 'admin' || profile?.role === 'super_admin'} isSuperAdmin={profile?.role === 'super_admin'} platformRole={profile?.role ?? 'user'} currentUserId={user.id} boardRole={membership?.role ?? null} />
}
