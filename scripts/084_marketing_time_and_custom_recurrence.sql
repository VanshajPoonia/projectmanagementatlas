-- Adds an optional time-of-day to marketing calendar posts, and threads it
-- through the atomic series-update RPC (078) as a shared, series-wide field —
-- the same treatment content/is_highlighted already get. Nullable/optional
-- throughout: existing and imported rows are unaffected.
--
-- The RPC is DROPped and recreated rather than CREATE OR REPLACEd: Postgres
-- identifies a function by name AND its parameter-type list, so appending a
-- parameter via CREATE OR REPLACE would create a second overload sitting
-- alongside the old 5-arg one, not replace it. A freshly created function
-- grants EXECUTE to PUBLIC by default, so the REVOKE/GRANT block below is
-- mandatory here, not just hardening — mirrors 078's own original grant step.
...
DROP FUNCTION IF EXISTS public.update_marketing_calendar_series_atomic(
  UUID, JSONB, TEXT, BOOLEAN, UUID[]
);

CREATE FUNCTION public.update_marketing_calendar_series_atomic(
