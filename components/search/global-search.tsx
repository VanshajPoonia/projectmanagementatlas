...
export default function GlobalSearch({ isAdmin }: GlobalSearchProps) {
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: taskRows }, { data: commentRows }] = await Promise.all([
        supabase
          .from('tasks')
          .select('id, title, description, priority, due_date, status, column:columns(board_id, status_key, board:boards(id, title, archived_at))')
          .is('deleted_at', null),
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-sm sm:max-w-xl">
