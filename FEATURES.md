...
- [x] **6. My Work + Inbox** (Prompt F, `120`-`122`, dev AND prod, 2026-08-28) - `/inbox`
      is a real route in the nav for every role, plus an unread bell in the topbar. Two buckets
      (Action required / Updates) and no more, with Snoozed and Muted as their own scopes so
      nothing a control hides becomes unfindable. Mark read/unread, snooze by duration, follow
      or mute a work item, mute a project, bursts deduplicated, and a comment notification deep
      links to the comment. **No new notifications table** - `task_notifications` (035) already
      had the right shape and RLS; `120` adds `snoozed_until` + `entity_type`/`entity_id` and
      two small preference tables. My Work grew four sections that used to be listed on screen
      as unanswerable (Blocked by others, Blocking others, Waiting on approval, Personal tasks)
...
- **2026-08-28** - **Prompt F: My Work sections, WorkNext signals, the Inbox and notification control (migrations `120`-`122`, dev AND prod).** All three are purely additive and `--allow-prod` eligible on the standing rule - unlike `113` and `118`, none needed an owner override. **Applied to prod 2026-08-28**, one file at a time with `--only=NNN --allow-prod`, verified between each, after a `pg_restore --list`-verified backup (55 tables). Prod went 119 -> 122, `pending: 0`, with **every row count identical before and after** (173 tasks, 11 boards, 10 profiles, 193 notifications / 133 unread). `120` and `122` seed nothing; `121` seeded exactly one status. ⚠️ **`121`'s live effect is 3 tasks on Marketing PM Sheet**, not the 1 the `112` note recorded - they keep category `started`, so no dashboard or report count moved; their assignees just see them under "Waiting on approval" now. **Audit first, and it found a stale claim rather than a conflict:** `lib/my-work.ts`'s `UNANSWERED_QUESTIONS` still told users that "what am I blocking?" needed task dependencies and "what needs approval?" needed an approvals module - `115` had shipped relations two migrations earlier. A documented limitation with no owner becomes a claim the code no longer supports. (1) **`120`** adds `snoozed_until` + `entity_type`/`entity_id` to `task_notifications` (035) and two small private tables, `task_follows` and `board_mutes`. **No new notifications table**: 035 already had inbox-shaped RLS, four writers and 84 rows on dev, and a second inbox would give everyone two unread counts that disagree. **No stored link path**, deliberately - a board lives at both `/admin/board/<id>` and `/dashboard/board/<id>` and which one a person may open depends on their role, so the href is composed at read time from `boardHref`. **Mute is enforced at READ and follow at WRITE**: unmuting therefore restores the history instead of having destroyed it, and a mute cannot leak the first time somebody adds a fifth writer. No admin bypass on any policy (asserted by a post-condition) - what you have muted is a statement about your own attention. Deprovisioning was decided at table creation this time, per 119's lesson: both CASCADE on `user_id` and neither needs the delete-user route. (2) **`121`** adds `task_statuses.is_approval` - not a sixth category, because work awaiting sign-off has genuinely started - and seeds exactly `key = 'pending_approval'` by exact match. Dev seeded 0 (no such status); **prod has one**, so applying it there will move one card into a "Waiting on approval" section. Reachable from Super Admin → Statuses on create and edit. (3) **`122`** is `notify_task_watchers`, SECURITY DEFINER, `authenticated`-only with `has_function_privilege()` assertions. **It exists because 120 made follows private**: the person writing a comment cannot see who follows the task, so a client asking would read the empty list as "nobody does" - "hidden from you" and "does not exist" arriving identical through a policy. (4) **Commenting sent email and no in-app notification at all** - four writers existed and none fired for a comment. Comments now notify watchers, and `@name` mentions get their own targeted notification in Action Required, resolved by quick capture's own `matchPerson` so `@bobby` cannot mean two different people on two screens; an ambiguous token is skipped rather than resolved to whoever sorts first. (5) **The update path notified assignees only**, so following a work item you were not assigned to could never have worked. (6) **My Work** grew Blocked by others, Blocking others, Waiting on approval, Personal tasks and Recently viewed, and section order/visibility is a per-user `localStorage` preference that REPAIRS what it reads - a new section is inserted at its default position rather than being invisible to everyone who ever opened the panel. (7) **`WorkNext` was extended, not replaced**: `scoreTask` takes optional signals, every existing caller keeps its exact ranking (pinned), and blocked/blocking/approval each carry a visible reason read from the same numbers as the score. **Verified:** 1573 unit tests green (87 new) under all four timezones via `pnpm test:timezones`, `tsc --noEmit` clean, `next build` clean with `.env.production.local` moved aside and the dev ref confirmed baked, `pnpm check:inbox` 49/49 (new, real RLS), `pnpm check:inbox-ui` 32/32 (new, real browser, stable across four consecutive runs), `pnpm check:my-work` 33/33 (was 20), and access-matrix / board-roles / deprovision / deactivation / grants / task-lifecycle / work-items / work-items-ui / views / views-ui / crm / teams / column-delete / task-move / board-nav all re-run green. `scripts/audit-mobile.mjs` now sweeps `/inbox` too: 34 of 51 flagged, **zero horizontal overflow anywhere**, and every flag on the new page is shell chrome shared with every other route. **Three defects only the real browser found**, all fixed and pinned: the unread badge did not follow the page it mirrors (marking all read left it saying 3); a slow initial follow-state read overwrote a fast click on Follow, failing one run in three; and `board_mutes` has no UPDATE grant, so supabase-js's default `ON CONFLICT DO UPDATE` upsert was refused for every user. **Not done, deliberately:** digest preferences (the prompt says "later"), and following a whole BOARD - that would need notification generation nothing currently does, and a control wired to nothing is this repo's most-repeated defect.
