...
// Validate + normalise client attachments. Returns null (with a reason) if anything
// is off, so we reject the whole request rather than send junk to Gemini.
...
export async function POST(request: Request) {
  try {
    ...
    if (mode === 'web' && !checkRateLimit(`ai-chat-web:${user.id}`, 25, 60 * 60 * 1000)) {
      return NextResponse.json({ error: "You've hit the hourly web-search limit — try again in a bit." }, { status: 429 })
    }
