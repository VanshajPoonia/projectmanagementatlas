...
export default async function UserBoardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <BoardView board={board} columns={columns || []} users={users || []} isAdmin={false} isSuperAdmin={profile?.role === 'super_admin'} platformRole={profile?.role ?? 'user'} currentUserId={user.id} boardRole={membership?.role ?? null} />
}
